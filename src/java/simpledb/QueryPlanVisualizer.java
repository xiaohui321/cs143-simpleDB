...
public class QueryPlanVisualizer {
    ...
    private int calculateQueryPlanTreeDepth(final DbIterator root) {
	if (root == null)
	    return 0;
	...
	if (o instanceof Join || o instanceof HashEquiJoin) {
	    int d1 = calculateQueryPlanTreeDepth(children[0]);
	    int d2 = calculateQueryPlanTreeDepth(children[1]);
	    return Math.max(d1, d2) + 3;
	} else if (children != null && children[0] != null)
	    return calculateQueryPlanTreeDepth(children[0]) + 2;
	return 2;
    }
	...
	SubTreeDescriptor(final SubTreeDescriptor leftChild,
	        final SubTreeDescriptor rightChild) {
    ...
    private SubTreeDescriptor buildTree(final int queryPlanDepth,
	    final int currentDepth, final DbIterator queryPlan,
	    final int currentStartPosition, final int parentUpperBarStartShift) {
	if (queryPlan == null)
	    return null;
	...
	if (queryPlan instanceof SeqScan) {
		...
		SubTreeDescriptor left = buildTree(queryPlanDepth, currentDepth
		        + adjustDepth + 3, children[0], currentStartPosition,
		        upBarShift);
		SubTreeDescriptor right = buildTree(queryPlanDepth,
		...
		thisNode.text = String.format("%1$s(%2$s),card:%3$d",
		        HASH_JOIN, field1 + jp.getOperator() + field2,
		        j.getEstimatedCardinality());
		int upBarShift = parentUpperBarStartShift;
		if (HASH_JOIN.length() / 2 > parentUpperBarStartShift)
		    upBarShift = HASH_JOIN.length() / 2;
		SubTreeDescriptor left = buildTree(queryPlanDepth, currentDepth
		        + 3 + adjustDepth, children[0], currentStartPosition,
		        upBarShift);
		SubTreeDescriptor right = buildTree(queryPlanDepth,
		...
		if (gfield == Aggregator.NO_GROUPING) {
		    thisNode.text = String.format("%1$s(%2$s),card:%3$d",
			    a.aggregateOp(), a.aggregateFieldName(),
			    a.getEstimatedCardinality());
		    alignTxt = td.getFieldName(00);
		} else {
		    thisNode.text = String
			    .format("%1$s(%2$s), %3$s(%4$s),card:%5$d",
			            GROUPBY, a.groupFieldName(),
			            a.aggregateOp(), a.aggregateFieldName(),
			            a.getEstimatedCardinality());
		    alignTxt = GROUPBY;
		}
		...
		SubTreeDescriptor child = buildTree(queryPlanDepth,
		...
		thisNode.text = String.format("%1$s(%2$s),card:%3$d", SELECT,
		        children[0].getTupleDesc().getFieldName(p.getField())
		...
		thisNode.text = String.format("%1$s(%2$s),card:%3$d", PROJECT,
		        fields, p.getEstimatedCardinality());
		int upBarShift = parentUpperBarStartShift;
		...
		thisNode.leftChild = child;
		thisNode.height = currentDepth;
	    } else if (plan.getClass().getSuperclass().getSuperclass()
		    .getSimpleName().equals("Exchange")) {
		String name = "Exchange";
		int card = 0;
		try {
		    name = (String) plan.getClass().getMethod("getName")
			    .invoke(plan);
		    card = (Integer) plan.getClass()
			    .getMethod("getEstimatedCardinality").invoke(plan);
		}
		catch (Exception e) {
		    e.printStackTrace();
		}
		...
		if (child == null) {
		    ...
		    thisNode.textStartPosition = thisNode.upBarPosition
			    - name.length() / 2;
		    thisNode.width = thisNode.textStartPosition
		...
		try {
		    newName = (String) plan.getClass()
			    .getMethod("newName", (Class<?>[]) null)
			    .invoke(plan);
		    fieldIdx = (Integer) plan.getClass()
			    .getMethod("renamedField", (Class<?>[]) null)
			    .invoke(plan);
		}
		catch (Exception e) {
		    e.printStackTrace();
		...
		thisNode.text = String.format("%1$s,%2$s->%3$s,card:%4$d",
    ...
    private void printTree(final SubTreeDescriptor root, final char[] buffer,
	    final int width) {
	if (root == null)
	    return;
	...
	SubTreeDescriptor root = buildTree(queryPlanDepth, 0, physicalPlan, 0,
	        0);
    ...
    public void printQueryPlanTree(final DbIterator physicalPlan,
	    PrintStream out) {
	if (out == null)
	    out = System.out;

	String tree = getQueryPlanTree(physicalPlan);
